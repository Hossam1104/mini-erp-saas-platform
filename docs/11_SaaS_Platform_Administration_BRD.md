...
| Version | 0.9 — Founder review candidate |
| Status | In Review — not approved for implementation |
...
| MESP-52 | Plan, module, limit, and entitlement decision is proposed in this BRD for Hossam's approval. |
...
| Plan | A named, reusable commercial package defining available modules, features, and limits. |
...
| Platform Operations Owner | Owns provisioning recovery, service limits, notifications, jobs, evidence, and operational readiness. | Safety overrides require reason, expiry, and review. |
...
4. Release 1 has one proposed Plan and no billing automation.
...
Reactivated is an explicit evidence state and must return to Active after restoration checks; it is not a permanent operating state. Some controlled transitions may skip Grace Period for security or legal suspension, but no state may skip required evidence.
...
| Purge Approved | Irreversible purge has passed legal, contractual, retention, backup, evidence, and dual-control checks. | Execute only the approved purge scope; pause on new hold or failed precondition. No reactivation. | Purged after verified completion; return to Retained if execution has not begun and approval is revoked. | No tenant access or Entitlements. Approval, scope, hashes/manifests, start, errors, and completion audited; required owners notified. |
| Purged | Approved tenant data and files are irreversibly removed from active systems according to policy; minimal non-sensitive proof remains. | View purge certificate and allowed platform accounting/audit proof. No restore, reuse, sign-in, export, or business lookup. | Terminal. | No data access or Entitlements. Evidence must not reconstruct purged business data; completion notice and immutable certificate retained. |
...
- **M27-REQ-015 (Must):** Purged is terminal; a restoration promise must never be made after verified irreversible purge.

## 11. Plan and subscription model

### Proposed MESP-52 Release 1 decision — requires Hossam approval

1. One named Release 1 Plan contains all approved B2B ERP modules.
2. Retail POS is absent and cannot be assigned.
3. The Plan carries simple configurable limits, but no price calculation, metered billing, overage charge, automatic invoice, or tier automation.
4. An authorized Platform/Commercial Administrator manually assigns the Plan to a Tenant.
5. Plan versions and Subscription changes are effective-dated, reasoned, and audited; historical Entitlements remain reproducible.

| Concept | Release 1 treatment |
|---|---|
| Plan | Reusable versioned package; one proposed active R1 Plan. |
...
| Commercial limit | Configured Plan/Tenant capacity requiring approval to change. |
| Operational safety limit | Protective operational ceiling with owner, reason, review, and expiry. |

- **M27-REQ-016 (Must):** A Plan version must have a unique identity, name, status, effective interval, included modules/features, limit definitions, owner, and approval record.
...
- **M27-REQ-023 (Must):** Every Entitlement must expose its source Plan version, Subscription, effective interval, status, reason for override if any, and audit history.
...
- Required numbering/configuration profile and onboarding owner.
...
- **M27-REQ-031 (Must):** A module may become Active only when the Tenant is Active, the Entitlement is effective, dependencies are Active/Ready, required configuration and opening evidence are complete, and an authorized actor approves activation.
...
Allowed branding: Arabic/English display names, approved logo, contact details, document identity, and colors within accessible governed choices. Branding cannot alter behavior, authorization, tax meaning, legal evidence, audit, error severity, or Platform ownership disclosures.
...
- **M27-REQ-062 (Must):** Purge requires dual control, verified retention expiry, no legal hold, exact Tenant/scope confirmation, backup treatment, failure recovery plan, and irreversible-action warning.
...
| M27-RULE-002 | Every Tenant-owned record, file, job, report, export, notification, audit event, and support action belongs to exactly one Tenant unless it is explicitly Platform-owned metadata. |
...
| M27-RULE-007 | A Permission cannot override missing Entitlement; an Entitlement cannot grant a User Permission. |
| M27-RULE-008 | A module cannot activate until its Entitlement, dependencies, configuration, opening evidence, and activation approval are valid. |
...
| M27-RULE-021 | Tenant export requires authorization at request, generation, and download; an expired artifact cannot be downloaded. |
...
| M27-RULE-024 | Purge is irreversible, dual-controlled, exact-scope, evidence-producing, and permitted only after MESP-50 controls are approved. |
...
| Purge | Candidate → On Hold/Eligible → Approved → Executing → Completed or Failed | Legal hold blocks approval/execution. Completed is irreversible; Failed requires reviewed recovery. |
...
| Plan version | Identity/name/version, status, modules/features, limit definitions, service metadata, effective interval, approval | Product/Commercial; historical versions immutable. |
...
| Entitlement | Tenant, capability/capacity, source, status, effective interval, override reason/authority | Platform; historical evaluation reproducible. |
| Provisioning request/run | Inputs, duplicate key, stages, attempts, results, errors, compensations, correlation, evidence | Operations; safe retry and audit required. |
| Module activation | Module, dependency/config checklist, data/open-process assessment, status, approver, effective time | Platform/Tenant shared evidence by responsibility. |
...
| Branding | Bilingual identity, logo metadata, colors, contacts, version, preview/validation, effective time | Tenant-owned within Platform controls. |
...
| Export/retention/purge | Scope, manifest, integrity, holds, approvals, artifacts, expiry/downloads, purge certificate | Privacy/Security; final retention set by MESP-50. |
...
3. Country Pack, Plan version, Subscription interval, module dependencies, and limit definitions exist and are eligible at the effective time.
...
8. Support access cannot exceed approved Tenant, purpose, scope, or time.
9. Export scope is authorized and bounded; download occurs before expiry.
10. Purge Tenant/scope is independently confirmed and has no active hold.
...
| Assign/change Plan | Commercial/Platform permission | Effective date, reason, approval |
| Activate Tenant/module | Platform activation permission | Readiness/activation checklist and Tenant acknowledgment |
...
| Request/generate/download export | Separate authorized rights | Recheck scope at every stage |
| Terminate | Contract/lifecycle authority | Export and legal-hold checks |
| Approve/execute purge | Two distinct authorized actors | MESP-50 policy and exact-scope confirmation |
...
- **M27-REQ-079 (Must):** KPIs must not imply committed service capacity or commercial entitlement before MESP-48/MESP-52 approval.
...
| Export fails or expires | Preserve request/manifest state; safe retry creates a new controlled artifact; old artifact remains inaccessible. |
...
|---|---|
| Availability/recovery | Platform lifecycle and evidence must remain recoverable; numeric commitments follow production topology and MESP-48 validation. |
| Performance | Common administration actions and reports must meet approved PRD targets under the later reference load. |
...
5. **M27-AC-005 — Plan assignment:** Given an eligible Draft/Configuration Required Tenant and approved R1 Plan version, when an authorized actor assigns it with effective dates, then the Subscription and staged Entitlements are recorded without user Permissions.
...
9. **M27-AC-009 — Permission without Entitlement:** Given a User has a module Permission but the Tenant lacks Entitlement, when the User acts, then access is denied without enabling the module.
...
11. **M27-AC-011 — Module activation:** Given Entitlement, dependencies, configuration, opening evidence, and approval are complete, when activation occurs, then the module becomes Active and the evidence snapshot is retained.
...
29. **M27-AC-029 — Export:** Given an authorized export request, when generation succeeds, then a bounded manifest/artifact with integrity, scope, expiry, and audit is available only to an authorized downloader.
...
33. **M27-AC-033 — Purge approval:** Given MESP-50 controls are approved, retention expired, no hold exists, and two authorized actors confirm exact scope, when purge is approved, then execution may start with irreversible-action evidence.
...
35. **M27-AC-035 — Purge evidence:** Given purge completes and verification passes, when the Tenant becomes Purged, then only the approved minimal non-sensitive certificate remains and restoration/export is impossible.
...
|---|---|---|---|
| M27-OQ-001 | Approve the proposed one-plan MESP-52 decision exactly as stated? | Hossam now, to approve this BRD | Blocks BRD approval, not drafting. |
...
| M27-OQ-006 | What retention periods, residency, legal-hold authority, backup treatment, and purge certificate content apply? | MESP-50 + qualified advisors before production | Blocks production offboarding/purge. |
...
| M27-DEC-001 — One R1 Plan with all approved B2B ERP modules, simple configurable limits, manual assignment, effective-dated audit, no metered/automated billing | **Proposed — Hossam approval required; intended answer to MESP-52** | Hossam | Allows Plan, Entitlement, module, limit, and lifecycle requirements to baseline. |
...
On approval of M27-DEC-001, create the next immutable Product Decision Register ID after PD-019 and update MESP-52; do not overwrite prior records.
...
| MESP-52 | Plan/Subscription/Entitlement decision | Resolve through founder approval of this BRD |
...
2. M27-DEC-001 is approved as the MESP-52 Release 1 decision or replaced with an explicit approved alternative.
...
| Approve M27-DEC-001 as the MESP-52 answer | Approve / Request changes |
...
**Status:** Keep **In Progress**. Add label `status-in-review`. Do not move to Done until Hossam approves `docs/92_MESP27_Founder_Review.md`. Do not start MESP-28.

**BRD review candidate completed:** `docs/11_SaaS_Platform_Administration_BRD.md`

The BRD contains the required 40-section business structure covering actors, tenant lifecycle, Plan/Subscription/Entitlement separation, provisioning, module activation/deactivation, configurable limits and usage, branding, support access, feature rollout, suspension/reactivation, offboarding, rules, state machines, data, validation, permissions, audit, notifications, reports, recovery, localization, security/privacy, integrations, migration/opening, NFR expectations, acceptance scenarios, open questions, decisions, dependencies, risks, and approval criteria.

**Proposed MESP-52 decision:** one Release 1 Plan containing all approved B2B ERP modules; no POS; simple configurable limits; manual Plan assignment; effective-dated and audited changes; no metered billing, automatic subscription invoicing, pricing/tiering automation, or overage billing. This is proposed, not approved, until Hossam accepts the founder review.

**MESP-48 evidence plan:** gather measured Wafra values plus one conservative SME reference profile for users, companies, branches, warehouses, products, suppliers/customers, documents/lines, files/storage, imports, exports, reports, jobs, integrations/API, and concurrency. No production threshold or capacity promise is approved without evidence and validation.

**Open decisions:** Grace Period duration; suspension/read-only policy by cause; support authorization/emergency policy; notification channels; MESP-48 values; MESP-50 retention, residency, legal hold, backup, purge, and residual-evidence policy; production provider decisions at their ADR gates.

**Top risks:** Entitlement/Permission confusion, arbitrary limits, interrupted work during suspension, standing support privilege, feature-flag debt, premature purge, founder role concentration, provider notification failures, and POS scope leakage.

**Traceability:** PLT-001/008/011/012/013/014; BR-001/003/004/010/011/015/016; RULE-001/002/016; PRD §17; MESP-27/48/52; PD-019; glossary and architecture baseline.

**Approval status:** Founder review pending in `docs/92_MESP27_Founder_Review.md`.

**Exact next workflow action:** keep MESP-27 **In Progress**, add `status-in-review`, request Hossam approval. After written approval, record the MESP-52 product decision under the next PD-NNN, update traceability, then move MESP-27 to Done. Do not create implementation Stories and do not start MESP-28 yet.
