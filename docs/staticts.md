...
**Last Updated:** 2026-08-17 10:27 +03:00
...
| MESP-143 | **Implemented at bounded repository scope on `feat/MESP-143-tenant-aware-entry`; Draft PR pending independent review.** Added normalized configuration-led Tenant host binding and common/platform resolution, trusted-proxy-only forwarded host behavior, exact server-side membership selection, safe no-access/platform boundaries, common-host canonical routing, Overview-first Angular entry, post-Overview Company/Branch operational context, generic Tenant branding/MESP fallback, and presentation-only SAR/currency metadata. Added 16 focused MESP-143 host/configuration/security tests; Angular is 204/204 across 23 spec files; production build passes at 490.85 kB initial with a 91.94 kB Supplier Quotation lazy chunk; Playwright remains 8/8 after the updated Overview-first shell journey; full backend is 748/770 with exactly 22 SQL safety cases environment-gated by the absent dedicated LocalDB variable. Owner assets, Tenant persistence/schema/migrations, DNS/TLS, Jira state, external providers, and downstream P2P/Finance/Inventory effects are unchanged. |
