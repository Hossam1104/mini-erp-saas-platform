...
**Last Updated:** 2026-08-13 19:50 +03:00
...
| MESP-121 | In Progress at its bounded Phase E local-runtime/sign-in correction handoff; Phase A Price List backend and Phase C Angular implementation remain on feat/MESP-121-price-list-b2b-pricing, with runtime implementation commit f9eff32, prior Phase D Jira evidence comment 11093, and draft PR #64 still open/unmerged. The tracked proxy remains the generic localhost:5000 fallback, while the new Development launcher generates ignored `.runtime/proxy.conf.json` for the selected backend (final local run: 5300) and starts Angular on 4300. Direct and frontend-origin MiniERP identity, sign-in/session/context/antiforgery/Price List, route/assets, OpenAPI, and Scalar smoke passed; planner acceptance, Opus/final review, merge, and closure remain pending. |
...
| Capability backlog | MESP-121 is In Progress at the Phase E runtime/sign-in correction handoff on draft PR #64; MESP-122-MESP-142 remain under existing Epics and are To Do/not activated. Planner acceptance, Opus/final review, merge, and Jira closure remain pending on the same branch/PR; MESP-121 is not closed by this handoff. |
...
| Source/production capability | MESP-121 Phase E verified the local integrated runtime over the existing Price List capability: safe port discovery, ignored generated proxy, exact direct/proxy target identity, Development cookie/sign-in/session/context behavior, and 689/689 non-SQL backend plus 55/55 Angular tests. PR #64 remains draft/unmerged; SQL/provider/production, migration, legal/privacy, specialist, planner, and final review gates remain open. Final project completion percentages remain unchanged. |
