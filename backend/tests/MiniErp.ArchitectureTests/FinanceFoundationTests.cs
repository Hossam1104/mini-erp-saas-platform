...
public sealed class FinanceFoundationTests
{
    ...
    public async Task Company_books_support_period_control_balanced_posting_reversal_and_gl_facts()
    {
        ...
        var approved = await persistence.TransitionJournalAsync(context, Action(submitted.Value!, "approve-1"), FinanceJournalStatus.Approved);
    ...
    public async Task Unbalanced_and_closed_period_journals_fail_only_at_posting()
    {
        ...
        var approved = await persistence.TransitionJournalAsync(context, Action(submitted.Value!, "unbalanced-approve"), FinanceJournalStatus.Approved);
        ...
        var closedApproved = await persistence.TransitionJournalAsync(context, Action(closedSubmitted.Value!, "closed-approve"), FinanceJournalStatus.Approved);
    ...
    public async Task Foreign_currency_requires_exact_active_master_data_rate_evidence()
    {
        ...
        rates.Record = new MasterDataExchangeRateRecord(
            ...
            1,
            [new MasterDataExchangeRateVersionRecord(versionId, 1, new DateOnly(2026, 1, 1), null, 3.75m, 4, ExchangeRateProvenance.Configured, "test", "SAR", "USD")],
            [1]);
    ...
    public async Task Posting_rules_and_cost_centers_fail_closed_when_dimension_evidence_is_missing_or_invalid()
    {
        ...
        var approved = await persistence.TransitionJournalAsync(context, Action(submitted.Value!, "missing-center-approve"), FinanceJournalStatus.Approved);
    ...
    private static FinancePersistence CreatePersistence(DbContextOptions options) =>
        new(options, Companies(), new UnavailableInventoryValuationPersistence(), new UnavailableMasterDataExchangeRatePersistence());
    ...
    private static FinanceRequestContext Context(string permission)
    {
