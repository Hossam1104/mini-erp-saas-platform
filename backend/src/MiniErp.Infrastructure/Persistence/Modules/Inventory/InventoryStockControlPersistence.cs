...
internal sealed partial class InventoryPersistence
{
    ...
    public Task<InventoryAdjustmentRecord?> ApproveAdjustmentAsync(InventoryRequestContext context, InventoryControlActionCommand command, CancellationToken cancellationToken = default) => MutateAdjustmentAsync(context, command, "inventory.adjustment.approve", (db, entity, now) =>
    {
        ...
        var stages = ReadApproval(entity.ApprovalPolicySnapshotJson)?.Stages.OrderBy(item => item.Sequence).ToArray(); var stage = stages?.ElementAtOrDefault(entity.CurrentApprovalStageIndex); if (stage is null) return false;
        var count = entity.CurrentStageApprovalCount + 1; var nextStage = count >= stage.RequiredApprovals && entity.CurrentApprovalStageIndex + 1 < stages!.Length ? entity.CurrentApprovalStageIndex + 1 : entity.CurrentApprovalStageIndex; var nextCount = nextStage == entity.CurrentApprovalStageIndex ? count : 0; entity.RecordApproval(command.ActorId, command.DelegatedFromActorId, nextStage, nextCount, now); if (nextStage == entity.CurrentApprovalStageIndex && count >= stage.RequiredApprovals) entity.SetStatus(InventoryControlDocumentStatus.Approved, now); return true;
    ...
    public Task<InventoryStockIssueRecord?> ApproveStockIssueAsync(InventoryRequestContext context, InventoryControlActionCommand command, CancellationToken cancellationToken = default) => MutateIssueAsync(context, command, "inventory.issue.approve", (entity, now) => { if (entity.Status != InventoryControlDocumentStatus.PendingApproval) return false; var stages = ReadApproval(entity.ApprovalPolicySnapshotJson)?.Stages.OrderBy(item => item.Sequence).ToArray(); var stage = stages?.ElementAtOrDefault(entity.CurrentApprovalStageIndex); if (stage is null) return false; var count = entity.CurrentStageApprovalCount + 1; var nextStage = count >= stage.RequiredApprovals && entity.CurrentApprovalStageIndex + 1 < stages!.Length ? entity.CurrentApprovalStageIndex + 1 : entity.CurrentApprovalStageIndex; var nextCount = nextStage == entity.CurrentApprovalStageIndex ? count : 0; entity.RecordApproval(command.ActorId, command.DelegatedFromActorId, nextStage, nextCount, now); if (nextStage == entity.CurrentApprovalStageIndex && count >= stage.RequiredApprovals) entity.SetStatus(InventoryControlDocumentStatus.Approved, now); return true; }, InventoryControlHistoryAction.Approved, cancellationToken);
    ...
    public async Task<InventoryCountRecord?> CreateCountAsync(InventoryRequestContext context, InventoryCountCreateCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var entity = new InventoryCountEntity(context.TenantId, command.Id, command.Scope.CompanyId, command.Scope.BranchId, command.Scope.WarehouseId, command.WarehouseCode, command.WarehouseName, command.CountType, command.AssignedCounterId, command.ReviewerId, command.SnapshotCutoff, command.ActorId, command.OccurredAt);
            foreach (var line in command.Lines)
            {
    ...
    public async Task<InventoryCountRecord?> SubmitCountAsync(InventoryRequestContext context, InventoryCountSubmitCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var observation in command.Observations) { var line = current[observation.CountLineId]; InventoryReasonCodeRecord? reason = null; var variance = observation.CountedQuantity - line.ExpectedQuantity; if (variance != 0m) { if (string.IsNullOrWhiteSpace(observation.VarianceReasonCode)) return null; var reasonEntity = await db.ReasonCodes.AsNoTracking().SingleOrDefaultAsync(item => item.Code == observation.VarianceReasonCode.Trim().ToUpperInvariant() && item.IsActive && item.Category == InventoryReasonCategory.CountVariance, cancellationToken); if (reasonEntity is null) return null; reason = ToReason(reasonEntity); } line.SetObservation(observation.CountedQuantity, reason, command.OccurredAt); }
            var hasVariance = current.Values.Any(item => item.Variance is not null and not 0m); var from = entity.Status; entity.MarkSubmitted(hasVariance ? InventoryControlDocumentStatus.PendingApproval : InventoryControlDocumentStatus.Posted, command.OccurredAt); if (!hasVariance) entity.MarkPosted(command.ActorId, command.OccurredAt); AddControlHistory(db, context, "count", entity.Id, null, InventoryControlHistoryAction.CountSubmitted, from, entity.Status, command.ActorId, null, null, command.CorrelationId, entity.CurrentRoundGeneration, command.OccurredAt); AddAudit(db, context, "count", entity.Id, "inventory.count.submit", command.ActorId, "Succeeded", null, command.CorrelationId, command.IdempotencyKey, command.RequestFingerprint, from.ToString(), entity.Status.ToString(), command.OccurredAt); await db.SaveChangesAsync(cancellationToken); var result = ToCount(entity, true); AddReplay(db, context, "inventory.count.submit", command.IdempotencyKey, command.RequestFingerprint, "count", entity.Id, result, command.OccurredAt); await db.SaveChangesAsync(cancellationToken); await transaction.CommitAsync(cancellationToken); return result;
        }
        catch (Exception exception) when (InventoryPersistenceExceptionClassifier.IsSqlServerContention(exception)) { return null; }
        catch (DbUpdateConcurrencyException) { return null; }
        catch (DbUpdateException exception) { throw InventoryPersistenceExceptionClassifier.Unavailable(exception); }
    ...
    public Task<InventoryCountRecord?> ApproveCountAsync(InventoryRequestContext context, InventoryControlActionCommand command, CancellationToken cancellationToken = default) => MutateCountAsync(context, command, "inventory.count.approve", (db, entity, now) => { if (entity.Status != InventoryControlDocumentStatus.PendingApproval || entity.AssignedCounterId == command.ActorId || entity.Lines.Where(item => item.RoundGeneration == entity.CurrentRoundGeneration).Any(item => item.CountedQuantity is null)) return false; entity.MarkApproved(command.ActorId, now); return true; }, InventoryControlHistoryAction.Approved, cancellationToken);
    ...
    public async Task<InventoryCountRecord?> ResnapshotCountAsync(InventoryRequestContext context, InventoryControlActionCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var replay = await ProbeReplayAsync<InventoryCountRecord>(db, context, "inventory.count.resnapshot", command.IdempotencyKey, command.RequestFingerprint, cancellationToken); if (replay.Outcome == InventoryReplayOutcome.Replay) return replay.Value; if (replay.Outcome == InventoryReplayOutcome.Conflict) return null; var entity = await db.Counts.Include(item => item.Lines).SingleOrDefaultAsync(item => item.Id == command.Id, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(command.ExpectedVersion) || entity.Status != InventoryControlDocumentStatus.ResnapshotRequired) return null; var current = entity.Lines.Where(item => item.RoundGeneration == entity.CurrentRoundGeneration).ToArray(); var identities = current.Select(StockIdentityKey.From).Distinct().ToArray(); await AcquireConcurrencyAnchorsAsync(db, context.TenantId, identities, cancellationToken); var cutoff = DateTimeOffset.UtcNow; entity.BeginNewRound(cutoff, command.OccurredAt); foreach (var line in current) { var expected = await SignedQuantityAsync(db, StockIdentityKey.From(line), cancellationToken); entity.Lines.Add(new InventoryCountLineEntity(context.TenantId, Guid.NewGuid(), entity.Id, line.Id, entity.CurrentRoundGeneration, line.ProductId, line.ProductSku, line.ProductName, line.UnitOfMeasureId, line.UnitOfMeasureCode, line.TrackingIdentity, expected)); } AddControlHistory(db, context, "count", entity.Id, null, InventoryControlHistoryAction.Resnapshot, InventoryControlDocumentStatus.ResnapshotRequired, entity.Status, command.ActorId, null, command.Reason, command.CorrelationId, entity.CurrentRoundGeneration, cutoff); AddAudit(db, context, "count", entity.Id, "inventory.count.resnapshot", command.ActorId, "Succeeded", command.Reason, command.CorrelationId, command.IdempotencyKey, command.RequestFingerprint, "ResnapshotRequired", "Draft", cutoff); await db.SaveChangesAsync(cancellationToken); var result = ToCount(entity, true); AddReplay(db, context, "inventory.count.resnapshot", command.IdempotencyKey, command.RequestFingerprint, "count", entity.Id, result, cutoff); await db.SaveChangesAsync(cancellationToken); await transaction.CommitAsync(cancellationToken); return result;
        }
        catch (Exception exception) when (InventoryPersistenceExceptionClassifier.IsSqlServerContention(exception)) { return null; }
        catch (DbUpdateConcurrencyException) { return null; }
        catch (DbUpdateException exception) { throw InventoryPersistenceExceptionClassifier.Unavailable(exception); }
    ...
    public async Task<InventoryCountRecord?> PostCountAsync(InventoryRequestContext context, InventoryControlActionCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var replay = await ProbeReplayAsync<InventoryCountRecord>(db, context, "inventory.count.post", command.IdempotencyKey, command.RequestFingerprint, cancellationToken); if (replay.Outcome == InventoryReplayOutcome.Replay) return replay.Value; if (replay.Outcome == InventoryReplayOutcome.Conflict) return null; var entity = await db.Counts.Include(item => item.Lines).SingleOrDefaultAsync(item => item.Id == command.Id, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(command.ExpectedVersion)) return null; if (entity.Status == InventoryControlDocumentStatus.Posted) return ToCount(entity, true); if (entity.Status != InventoryControlDocumentStatus.Approved) return null;
            ...
            await AcquireConcurrencyAnchorsAsync(db, context.TenantId, identities, cancellationToken);
            List<StockIdentityKey> recentMovements;
            if (db.Database.IsSqlite())
            {
                var posted = await db.StockMovements.AsNoTracking()
                    .Select(item => new { item.CompanyId, item.BranchId, item.WarehouseId, item.ProductId, item.UnitOfMeasureId, item.TrackingIdentity, item.PostedAt })
                    .ToListAsync(cancellationToken);
                recentMovements = posted
            ...
            {
                recentMovements = await db.StockMovements
                    .AsNoTracking()
                    .Where(item => item.PostedAt > entity.SnapshotCutoff)
                    .Select(item => new StockIdentityKey(item.CompanyId, item.BranchId, item.WarehouseId, item.ProductId, item.UnitOfMeasureId, item.TrackingIdentity ?? string.Empty))
                    .ToListAsync(cancellationToken);
            }
            if (recentMovements.Any(item => identities.Contains(item)))
            {
                entity.SetStatus(InventoryControlDocumentStatus.ResnapshotRequired, command.OccurredAt);
                AddControlHistory(db, context, "count", entity.Id, null, InventoryControlHistoryAction.PostBlocked, InventoryControlDocumentStatus.Approved, entity.Status, command.ActorId, null, "resnapshot_required", command.CorrelationId, entity.CurrentRoundGeneration, command.OccurredAt);
                AddAudit(db, context, "count", entity.Id, "inventory.count.post", command.ActorId, "Failed", "resnapshot_required", command.CorrelationId, command.IdempotencyKey, command.RequestFingerprint, "Approved", "ResnapshotRequired", command.OccurredAt);
                await db.SaveChangesAsync(cancellationToken);
                await transaction.CommitAsync(cancellationToken);
                return ToCount(entity, true);
            }
            var negative = current.Where(item => item.Variance < 0m).GroupBy(StockIdentityKey.From).ToDictionary(group => group.Key, group => group.Sum(item => Math.Abs(item.Variance!.Value))); var availability = await ReadAvailabilityByIdentityAsync(db, identities, cancellationToken); if (negative.Any(item => availability[item.Key].OnHand - item.Value < availability[item.Key].Reserved)) { entity.SetStatus(InventoryControlDocumentStatus.Blocked, command.OccurredAt); AddControlHistory(db, context, "count", entity.Id, null, InventoryControlHistoryAction.PostBlocked, InventoryControlDocumentStatus.Approved, entity.Status, command.ActorId, null, "reservation_reconciliation_required", command.CorrelationId, entity.CurrentRoundGeneration, command.OccurredAt); AddAudit(db, context, "count", entity.Id, "inventory.count.post", command.ActorId, "Failed", "reservation_reconciliation_required", command.CorrelationId, command.IdempotencyKey, command.RequestFingerprint, "Approved", "Blocked", command.OccurredAt); await db.SaveChangesAsync(cancellationToken); await transaction.CommitAsync(cancellationToken); return entity is null ? null : ToCount(entity, true); }
            ...
            entity.MarkPosted(command.ActorId, command.OccurredAt); AddControlHistory(db, context, "count", entity.Id, null, InventoryControlHistoryAction.Posted, InventoryControlDocumentStatus.Approved, entity.Status, command.ActorId, null, command.Reason, command.CorrelationId, entity.CurrentRoundGeneration, command.OccurredAt); AddAudit(db, context, "count", entity.Id, "inventory.count.post", command.ActorId, "Succeeded", command.Reason, command.CorrelationId, command.IdempotencyKey, command.RequestFingerprint, "Approved", "Posted", command.OccurredAt); await db.SaveChangesAsync(cancellationToken); var result = ToCount(entity, true); AddReplay(db, context, "inventory.count.post", command.IdempotencyKey, command.RequestFingerprint, "count", entity.Id, result, command.OccurredAt); await db.SaveChangesAsync(cancellationToken); await transaction.CommitAsync(cancellationToken); return result;
        }
        catch (Exception exception) when (InventoryPersistenceExceptionClassifier.IsSqlServerContention(exception)) { return null; }
        catch (DbUpdateConcurrencyException) { return null; }
        catch (DbUpdateException exception) { throw InventoryPersistenceExceptionClassifier.Unavailable(exception); }
    ...
    private static PurchaseRequestApprovalPolicyDefinition? ReadApproval(string? json) => string.IsNullOrWhiteSpace(json) ? null : JsonSerializer.Deserialize<PurchaseRequestApprovalPolicyDefinition>(json, InventoryControlJson.Options);
