    ...
    : TenantPersistenceDbContext(options, tenantContext, TenantOwnershipVerifierRegistry.CreateInventory())
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        customerReturn.Property(item => item.SalesCustomerReturnId).IsRequired(); customerReturn.Property(item => item.CompanyId).IsRequired(); customerReturn.Property(item => item.BranchId).IsRequired(false); customerReturn.Property(item => item.WarehouseId).IsRequired(); customerReturn.Property(item => item.Status).IsRequired(); customerReturn.Property(item => item.PhysicalEvidenceReference).HasMaxLength(2048).IsRequired(); customerReturn.Property(item => item.InspectionEvidenceReference).HasMaxLength(2048).IsRequired(); customerReturn.Property(item => item.HandoffState).HasMaxLength(64).IsRequired(); customerReturn.Property(item => item.ReceiptDate).IsRequired(false); customerReturn.Property(item => item.PostedAt).IsRequired(false); customerReturn.Property(item => item.ActorId).IsRequired(); customerReturn.Property(item => item.CreatedAt).IsRequired(); customerReturn.Property(item => item.UpdatedAt).IsRequired(); customerReturn.HasIndex(item => new { item.TenantId, item.SalesCustomerReturnId }).IsUnique();
