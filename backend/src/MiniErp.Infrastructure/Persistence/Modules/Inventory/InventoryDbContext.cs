    ...
    : TenantPersistenceDbContext(options, tenantContext, TenantOwnershipVerifierRegistry.CreateInventory())
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        valuationPolicy.Property(item => item.CompanyId).IsRequired(); valuationPolicy.Property(item => item.FunctionalCurrencyId).IsRequired(); valuationPolicy.Property(item => item.FunctionalCurrencyCode).HasMaxLength(16).IsRequired(); valuationPolicy.Property(item => item.ScopeMode).IsRequired(); valuationPolicy.Property(item => item.EffectiveFrom).IsRequired(); valuationPolicy.Property(item => item.EffectiveTo).IsRequired(false); valuationPolicy.Property(item => item.VersionNumber).IsRequired(); valuationPolicy.Property(item => item.UnitCostScale).IsRequired(); valuationPolicy.Property(item => item.AmountScale).IsRequired(); valuationPolicy.Property(item => item.RoundingMode).IsRequired(); valuationPolicy.Property(item => item.GoodsReceiptCostBasis).HasMaxLength(128).IsRequired(); valuationPolicy.Property(item => item.PositiveAdjustmentCostBasis).HasMaxLength(128).IsRequired(); valuationPolicy.Property(item => item.SupplierReturnCostBasis).HasMaxLength(128).IsRequired(); valuationPolicy.Property(item => item.IsActive).IsRequired(); valuationPolicy.Property(item => item.ActorId).IsRequired(); valuationPolicy.Property(item => item.CreatedAt).IsRequired(); valuationPolicy.Property(item => item.UpdatedAt).IsRequired(); valuationPolicy.HasIndex(item => new { item.TenantId, item.Id }).IsUnique(); valuationPolicy.HasIndex(item => new { item.TenantId, item.CompanyId, item.EffectiveFrom, item.EffectiveTo });

        var scopeAnchor = modelBuilder.Entity<InventoryValuationScopeAnchorEntity>();
        ConfigureBase(scopeAnchor, "ValuationScopeAnchors");
        scopeAnchor.Property(item => item.CompanyId).IsRequired(); scopeAnchor.Property(item => item.BranchId).IsRequired(false); scopeAnchor.Property(item => item.WarehouseId).IsRequired(); scopeAnchor.Property(item => item.ProductId).IsRequired(); scopeAnchor.Property(item => item.UnitOfMeasureId).IsRequired(); scopeAnchor.Property(item => item.TrackingIdentity).HasMaxLength(256).IsRequired(); scopeAnchor.Property(item => item.PolicyId).IsRequired(); scopeAnchor.HasIndex(item => new { item.TenantId, item.PolicyId, item.CompanyId, item.BranchId, item.WarehouseId, item.ProductId, item.UnitOfMeasureId, item.TrackingIdentity }).IsUnique().HasFilter(null); scopeAnchor.HasIndex(item => new { item.TenantId, item.Id }).IsUnique();

        var valuationState = modelBuilder.Entity<InventoryValuationStateEntity>();
        ConfigureBase(valuationState, "ValuationStates");
        valuationState.Property(item => item.CompanyId).IsRequired(); valuationState.Property(item => item.BranchId).IsRequired(false); valuationState.Property(item => item.WarehouseId).IsRequired(); valuationState.Property(item => item.ProductId).IsRequired(); valuationState.Property(item => item.UnitOfMeasureId).IsRequired(); valuationState.Property(item => item.TrackingIdentity).HasMaxLength(256).IsRequired(); valuationState.Property(item => item.PolicyId).IsRequired(); valuationState.Property(item => item.PolicyVersionNumber).IsRequired(); valuationState.Property(item => item.FunctionalCurrencyCode).HasMaxLength(16).IsRequired(); valuationState.Property(item => item.Quantity).HasPrecision(28, 8).IsRequired(); valuationState.Property(item => item.Value).HasPrecision(28, 8).IsRequired(); valuationState.Property(item => item.AverageUnitCost).HasPrecision(28, 8).IsRequired(); valuationState.Property(item => item.LastAppliedLedgerSequence).IsRequired(); valuationState.Property(item => item.UpdatedAt).IsRequired(); valuationState.HasIndex(item => new { item.TenantId, item.PolicyId, item.CompanyId, item.BranchId, item.WarehouseId, item.ProductId, item.UnitOfMeasureId, item.TrackingIdentity }).IsUnique().HasFilter(null); valuationState.HasIndex(item => new { item.TenantId, item.Id }).IsUnique();
