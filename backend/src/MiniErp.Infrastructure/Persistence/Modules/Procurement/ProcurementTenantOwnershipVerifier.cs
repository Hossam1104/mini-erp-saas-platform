...
internal static class ProcurementTenantOwnershipVerifier
{
    ...
    {
        if (context is not ProcurementDbContext procurementContext
        ...
        var stored = procurementContext.Set<TEntity>()
        ...
        where TEntity : class, ITenantOwned
    {
        ...
        var stored = await procurementContext.Set<TEntity>()
            .Where(candidate => EF.Property<Guid>(candidate, "Id") == id)
            .Select(candidate => candidate.TenantId)
