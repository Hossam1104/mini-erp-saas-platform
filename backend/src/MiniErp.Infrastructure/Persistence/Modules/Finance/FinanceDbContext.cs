...
internal sealed class FinanceDbContext : TenantPersistenceDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        account.HasOne<FinanceAccountEntity>().WithMany().HasForeignKey(item => new { item.TenantId, item.CompanyId, item.ParentAccountId }).HasPrincipalKey(item => new { item.TenantId, item.CompanyId, item.Id }).OnDelete(DeleteBehavior.Restrict).IsRequired(false);
