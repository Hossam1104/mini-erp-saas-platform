...
internal sealed class FinancePersistence(
    ...
    IMasterDataExchangeRatePersistence exchangeRates) : IFinancePersistence
{
    ...
    public async Task<FinanceOperationResult<FinanceAccountRecord>> EditAccountAsync(FinanceRequestContext context, FinanceAccountCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.Accounts.SingleOrDefaultAsync(item => item.Id == command.Id && item.CompanyId == command.CompanyId, cancellationToken);
        if (entity is null || !entity.Version.SequenceEqual(command.ExpectedVersion)) return Failure<FinanceAccountRecord>("concurrency_conflict");
    ...
    public async Task<FinanceOperationResult<FinanceAccountRecord>> SetAccountLifecycleAsync(FinanceRequestContext context, Guid accountId, Guid companyId, FinanceAccountLifecycle lifecycle, byte[] expectedVersion, string idempotencyKey, string fingerprint, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.Accounts.SingleOrDefaultAsync(item => item.Id == accountId && item.CompanyId == companyId, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(expectedVersion)) return Failure<FinanceAccountRecord>("concurrency_conflict");
    ...
    { await using var db = CreateContext(context); return (await db.FiscalYears.AsNoTracking().Where(item => item.CalendarId == calendarId).OrderBy(item => item.YearNumber).ToListAsync(cancellationToken)).Select(ToYear).ToArray(); }
    ...
    public async Task<FinanceOperationResult<FinanceFiscalYearRecord>> CreateYearAsync(FinanceRequestContext context, FinanceFiscalYearCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var calendar = await db.Calendars.SingleOrDefaultAsync(item => item.Id == command.CalendarId && item.Lifecycle == FinanceCalendarLifecycle.Active, cancellationToken); if (calendar is null) return Failure<FinanceFiscalYearRecord>("calendar_not_configured");
    ...
    { await using var db = CreateContext(context); return (await db.FiscalPeriods.AsNoTracking().Where(item => item.FiscalYearId == fiscalYearId).OrderBy(item => item.Sequence).ToListAsync(cancellationToken)).Select(ToPeriod).ToArray(); }
    ...
    public async Task<FinanceOperationResult<FinanceFiscalPeriodRecord>> CreatePeriodAsync(FinanceRequestContext context, FinanceFiscalPeriodCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var year = await db.FiscalYears.SingleOrDefaultAsync(item => item.Id == command.FiscalYearId, cancellationToken); if (year is null || command.StartDate < year.StartDate || command.EndDate > year.EndDate) return Failure<FinanceFiscalPeriodRecord>("fiscal_year_scope_invalid");
    ...
    public async Task<FinanceOperationResult<FinanceFiscalPeriodRecord>> SetPeriodStateAsync(FinanceRequestContext context, FinancePeriodStateCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.FiscalPeriods.SingleOrDefaultAsync(item => item.Id == command.PeriodId, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(command.ExpectedVersion)) return Failure<FinanceFiscalPeriodRecord>("concurrency_conflict");
    ...
    { await using var db = CreateContext(context); return (await db.CostCenters.AsNoTracking().Where(item => item.CompanyId == companyId).OrderBy(item => item.Code).ToListAsync(cancellationToken)).Select(ToCostCenter).ToArray(); }
    ...
    { await using var db = CreateContext(context); return (await db.PostingRules.AsNoTracking().Where(item => item.CompanyId == companyId).OrderBy(item => item.SourceContract).ThenBy(item => item.SourceEvent).ThenBy(item => item.VersionNumber).ToListAsync(cancellationToken)).Select(ToRule).ToArray(); }
    ...
    public async Task<FinanceOperationResult<FinancePostingRuleRecord>> SetPostingRuleLifecycleAsync(FinanceRequestContext context, Guid ruleId, Guid companyId, FinancePostingRuleLifecycle lifecycle, byte[] expectedVersion, string idempotencyKey, string fingerprint, CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null || !entity.Version.SequenceEqual(expectedVersion)) return Failure<FinancePostingRuleRecord>("concurrency_conflict");
    ...
    { await using var db = CreateContext(context); return (await db.Journals.AsNoTracking().Include(item => item.Lines).Where(item => item.CompanyId == companyId).OrderByDescending(item => item.JournalSequence).Take(500).ToListAsync(cancellationToken)).Select(ToJournal).ToArray(); }
    ...
    public async Task<FinanceOperationResult<FinanceJournalRecord>> CreateJournalAsync(FinanceRequestContext context, FinanceJournalCommand command, CancellationToken cancellationToken = default)
    {
        ...
        for (var index = 0; index < command.Lines.Count; index++) { var line = command.Lines[index]; if (!accounts.TryGetValue(line.AccountId, out var account)) return Failure<FinanceJournalRecord>("account_not_found"); if (line.CostCenterId is { } costCenterId && (!centers.TryGetValue(costCenterId, out var center) || !DimensionIsEffective(center, command.PostingDate))) return Failure<FinanceJournalRecord>("dimension_invalid"); centers.TryGetValue(line.CostCenterId ?? Guid.Empty, out var selectedCenter); var functional = FunctionalAmounts(line, company.FunctionalCurrencyCode, command.TransactionCurrencyCode, command.ExchangeRate); if (!functional.Valid) return Failure<FinanceJournalRecord>(functional.Code); entity.Lines.Add(new FinanceJournalLineEntity(context.TenantId, Guid.NewGuid(), entity.Id, index + 1, account, line, selectedCenter, functional.Debit, functional.Credit)); }
    ...
    public async Task<FinanceOperationResult<FinanceJournalRecord>> EditJournalAsync(FinanceRequestContext context, FinanceJournalCommand command, byte[] expectedVersion, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.Journals.Include(item => item.Lines).SingleOrDefaultAsync(item => item.Id == command.Id && item.CompanyId == command.CompanyId, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(expectedVersion)) return Failure<FinanceJournalRecord>("concurrency_conflict"); if (entity.Status is not (FinanceJournalStatus.Draft or FinanceJournalStatus.Rejected)) return Failure<FinanceJournalRecord>("journal_immutable");
        ...
        foreach (var (line, index) in command.Lines.Select((value, index) => (value, index))) { if (!accounts.TryGetValue(line.AccountId, out var account)) return Failure<FinanceJournalRecord>("account_not_found"); if (line.CostCenterId is { } costCenterId && (!centers.TryGetValue(costCenterId, out var center) || !DimensionIsEffective(center, command.PostingDate))) return Failure<FinanceJournalRecord>("dimension_invalid"); centers.TryGetValue(line.CostCenterId ?? Guid.Empty, out var selectedCenter); var functional = FunctionalAmounts(line, company.FunctionalCurrencyCode, command.TransactionCurrencyCode, command.ExchangeRate); if (!functional.Valid) return Failure<FinanceJournalRecord>(functional.Code); lines.Add(new FinanceJournalLineEntity(context.TenantId, Guid.NewGuid(), entity.Id, index + 1, account, line, selectedCenter, functional.Debit, functional.Credit)); }
    ...
    public async Task<FinanceOperationResult<FinanceJournalRecord>> TransitionJournalAsync(FinanceRequestContext context, FinanceJournalActionCommand command, FinanceJournalStatus target, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.Journals.Include(item => item.Lines).SingleOrDefaultAsync(item => item.Id == command.JournalId, cancellationToken); if (entity is null || !entity.Version.SequenceEqual(command.ExpectedVersion)) return Failure<FinanceJournalRecord>("concurrency_conflict");
    ...
    public async Task<FinanceOperationResult<FinanceJournalRecord>> ReverseJournalAsync(FinanceRequestContext context, FinanceReversalCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var original = await db.Journals.Include(item => item.Lines).SingleOrDefaultAsync(item => item.Id == command.JournalId, cancellationToken); if (original is null || original.Status != FinanceJournalStatus.Posted) return Failure<FinanceJournalRecord>("journal_not_posted"); if (original.ReversalJournalId is not null) return Failure<FinanceJournalRecord>("journal_already_reversed"); if (string.IsNullOrWhiteSpace(command.Reason)) return Failure<FinanceJournalRecord>("reason_required");
        var reversalCommand = new FinanceJournalCommand(original.CompanyId, original.JournalDate, command.PostingDate, original.TransactionCurrencyCode, original.ExchangeRate, original.ExchangeRateId, original.ExchangeRateVersionId, original.ExchangeRateVersionNumber, "finance-reversal.v1", "reversal", null, null, null, command.Reason, original.Lines.OrderBy(item => item.LineNumber).Select(item => new FinanceJournalLineCommand(item.AccountId, item.Credit, item.Debit, item.TransactionAmount.HasValue ? -item.TransactionAmount.Value : null, item.TransactionCurrencyCode, item.CostCenterId, item.Description)).ToArray(), command.Id, command.IdempotencyKey, command.RequestFingerprint);
        ...
        foreach (var sourceLine in original.Lines.OrderBy(item => item.LineNumber)) { var account = await db.Accounts.SingleAsync(item => item.Id == sourceLine.AccountId, cancellationToken); FinanceCostCenterEntity? center = null; if (sourceLine.CostCenterId is { } centerId) center = await db.CostCenters.SingleOrDefaultAsync(item => item.Id == centerId, cancellationToken); var line = new FinanceJournalLineCommand(sourceLine.AccountId, sourceLine.Credit, sourceLine.Debit, sourceLine.TransactionAmount.HasValue ? -sourceLine.TransactionAmount.Value : null, sourceLine.TransactionCurrencyCode, sourceLine.CostCenterId, sourceLine.Description); reversal.Lines.Add(new FinanceJournalLineEntity(context.TenantId, Guid.NewGuid(), reversal.Id, sourceLine.LineNumber, account, line, center, sourceLine.FunctionalCredit, sourceLine.FunctionalDebit)); }
        db.Journals.Add(reversal); reversal.SetStatus(FinanceJournalStatus.Approved, context.ActorId, DateTimeOffset.UtcNow); var validation = await ValidatePostingAsync(db, context, reversal, cancellationToken); if (!validation.Succeeded) return Failure<FinanceJournalRecord>(validation.Code); reversal.SetPeriod(validation.YearId, validation.PeriodId); reversal.SetStatus(FinanceJournalStatus.Posted, context.ActorId, DateTimeOffset.UtcNow); original.LinkReversal(reversal.Id); original.SetStatus(FinanceJournalStatus.Reversed, context.ActorId, DateTimeOffset.UtcNow); var now = DateTimeOffset.UtcNow; AddAudit(db, context, "finance.journal.reverse", "journal", original.Id, "Succeeded", command.Reason, command.IdempotencyKey, now); await db.SaveChangesAsync(cancellationToken); var result = FinanceOperationResult<FinanceJournalRecord>.Success(ToJournal(reversal)); AddReplay(db, context, "finance.journal.reverse", command.IdempotencyKey, command.RequestFingerprint, "journal", reversal.Id, result.Value!, now); await db.SaveChangesAsync(cancellationToken); await transaction.CommitAsync(cancellationToken); return result;
    }
    ...
    {
        var inventoryContext = context.ToInventoryRequestContext(); InventoryFinanceValuationHandoffRecord? handoff = null; foreach (var company in companies.List(context.TenantId)) { handoff = (await inventory.ListFinanceHandoffsAsync(inventoryContext, new InventoryValuationQuery(company.CompanyId), cancellationToken)).SingleOrDefault(item => item.Id == command.HandoffId); if (handoff is not null) break; } if (handoff is null) return Failure<FinanceJournalRecord>("handoff_not_found"); if (handoff.Status != InventoryFinanceValuationHandoffStatus.ReadyForFinance) return Failure<FinanceJournalRecord>("handoff_not_ready");
        ...
        var rule = await db.PostingRules.Where(item => item.CompanyId == handoff.CompanyId && item.SourceContract == InventoryContract && item.SourceEvent == handoff.SourceType.ToString() && item.Lifecycle == FinancePostingRuleLifecycle.Enabled && item.EffectiveFrom <= DateOnly.FromDateTime(handoff.AsOf.UtcDateTime) && (item.EffectiveTo == null || item.EffectiveTo >= DateOnly.FromDateTime(handoff.AsOf.UtcDateTime))).SingleOrDefaultAsync(cancellationToken); if (rule is null) return Failure<FinanceJournalRecord>("pending_mapping");
        var amount = Math.Abs(handoff.SignedBaseAmount); var commandId = existing?.Id ?? Guid.NewGuid(); var journalCommand = new FinanceJournalCommand(handoff.CompanyId, DateOnly.FromDateTime(handoff.AsOf.UtcDateTime), DateOnly.FromDateTime(handoff.AsOf.UtcDateTime), handoff.TransactionCurrencyCode ?? handoff.FunctionalCurrencyCode, handoff.TransactionCurrencyCode is null ? 1m : handoff.ExchangeRate, handoff.ExchangeRateId, handoff.ExchangeRateVersionId, handoff.ExchangeRateVersionNumber, InventoryContract, handoff.SourceType.ToString(), handoff.ValuationEvidenceId, handoff.ValuationEvidenceVersion, rule.Id, $"Inventory valuation {handoff.LedgerSequence}", [new FinanceJournalLineCommand(rule.DebitAccountId, amount, 0m, handoff.TransactionCurrencyCode is null ? amount : amount, handoff.TransactionCurrencyCode ?? handoff.FunctionalCurrencyCode, null, "Inventory valuation debit"), new FinanceJournalLineCommand(rule.CreditAccountId, 0m, amount, handoff.TransactionCurrencyCode is null ? amount : amount, handoff.TransactionCurrencyCode ?? handoff.FunctionalCurrencyCode, null, "Inventory valuation credit")], commandId, command.IdempotencyKey + ":create", command.RequestFingerprint);
        var created = existing is null ? await CreateJournalAsync(context, journalCommand, cancellationToken) : FinanceOperationResult<FinanceJournalRecord>.Success(ToJournal(existing)); if (!created.Succeeded || created.Value is null) return created; var current = created.Value; if (current.Status == FinanceJournalStatus.Draft) { var approved = await TransitionJournalAsync(context, new FinanceJournalActionCommand(current.Id, current.Version, "inventory handoff approval", command.IdempotencyKey + ":approve", command.RequestFingerprint), FinanceJournalStatus.Approved, cancellationToken); if (!approved.Succeeded || approved.Value is null) return approved; current = approved.Value; } return await PostJournalAsync(context, new FinanceJournalActionCommand(current.Id, current.Version, "inventory handoff post", command.IdempotencyKey + ":post", command.RequestFingerprint), cancellationToken);
    }
    ...
    {
        var company = Company(context, command.CompanyId); if (company is null) return (false, "company_scope_denied"); if (command.Lines is null || command.Lines.Count < 2) return (false, "journal_requires_two_lines"); if (command.Lines.Any(line => line.Debit < 0m || line.Credit < 0m || (line.Debit == 0m && line.Credit == 0m) || (line.Debit > 0m && line.Credit > 0m))) return (false, "journal_line_side_invalid"); if (command.Lines.Sum(line => line.Debit) == 0m && command.Lines.Sum(line => line.Credit) == 0m) return (false, "journal_zero_amount");
        ...
        if (currency != company.FunctionalCurrencyCode)
        {
            ...
            if (exchangeRate is null
                ...
                || version is null
                || !string.Equals(exchangeRate.SourceCurrencyCode, company.FunctionalCurrencyCode, StringComparison.OrdinalIgnoreCase)
                || !string.Equals(exchangeRate.TargetCurrencyCode, currency, StringComparison.OrdinalIgnoreCase)
    ...
    {
        if (entity.Status != FinanceJournalStatus.Approved) return PostingValidation.Failure("journal_must_be_approved"); var period = await db.FiscalPeriods.Where(item => item.CompanyId == entity.CompanyId && item.StartDate <= entity.PostingDate && item.EndDate >= entity.PostingDate).SingleOrDefaultAsync(cancellationToken); if (period is null) return PostingValidation.Failure("period_not_configured"); if (period.State != FinanceFiscalPeriodState.Open) return PostingValidation.Failure(period.State == FinanceFiscalPeriodState.SoftClosed ? "period_soft_closed" : "period_closed");
        var accountIds = entity.Lines.Select(line => line.AccountId).Distinct().ToArray(); var accounts = await db.Accounts.Where(item => item.CompanyId == entity.CompanyId && accountIds.Contains(item.Id)).ToDictionaryAsync(item => item.Id, cancellationToken); if (accounts.Count != accountIds.Length || entity.Lines.Any(line => !accounts[line.AccountId].IsPostingAccount || accounts[line.AccountId].Lifecycle != FinanceAccountLifecycle.Active || accounts[line.AccountId].EffectiveFrom > entity.PostingDate || accounts[line.AccountId].EffectiveTo < entity.PostingDate)) return PostingValidation.Failure("account_not_postable"); var centerIds = entity.Lines.Where(line => line.CostCenterId.HasValue).Select(line => line.CostCenterId!.Value).Distinct().ToArray(); var centers = await db.CostCenters.Where(item => item.CompanyId == entity.CompanyId && centerIds.Contains(item.Id)).ToDictionaryAsync(item => item.Id, cancellationToken); if (centers.Count != centerIds.Length || centers.Values.Any(center => !DimensionIsEffective(center, entity.PostingDate))) return PostingValidation.Failure("dimension_invalid"); var debit = entity.Lines.Sum(item => item.FunctionalDebit); var credit = entity.Lines.Sum(item => item.FunctionalCredit); if (debit != credit || debit <= 0m) return PostingValidation.Failure("journal_not_balanced");
    ...
    private FinanceCompanyOption? Company(FinanceRequestContext context, Guid companyId) => companies.List(context.TenantId).SingleOrDefault(item => item.CompanyId == companyId && item.IsActive);
    ...
    private static FinanceHandoffRecord ToHandoff(InventoryFinanceValuationHandoffRecord item, FinanceSourceEffectEntity? effect, IReadOnlyList<FinanceJournalEntity> journals) { var journal = effect is null ? null : journals.SingleOrDefault(value => value.Id == effect.JournalId); return new(item.Id, item.TenantId, item.CompanyId, item.MovementId, item.LedgerSequence, item.SourceType.ToString(), item.SourceDocumentId, item.SourceLineId, item.ValuationEvidenceId, item.ValuationEvidenceVersion, item.Quantity, item.Direction.ToString(), item.BaseUnitCost, item.BaseAmount, item.SignedBaseAmount, item.RoundingAdjustmentAmount, item.FunctionalCurrencyCode, item.TransactionCurrencyCode, item.ExchangeRateId, item.ExchangeRateVersionId, item.ExchangeRateVersionNumber, item.PolicyId, item.PolicyVersionNumber, item.CorrectionOfMovementId, effect is not null ? FinanceSourceHandoffStatus.Posted : item.Status == InventoryFinanceValuationHandoffStatus.ReadyForFinance ? FinanceSourceHandoffStatus.Ready : FinanceSourceHandoffStatus.PendingMapping, journal?.Id, item.ContractVersion, item.CorrelationId, item.AsOf, item.Version); }
}
