...
public sealed class FinanceMesp134Persistence : IFinanceMesp134Persistence
{
    ...
    public async Task<IReadOnlyList<FinanceTaxAccountingReconciliationRecord>> ReconcileTaxAsync(FinanceRequestContext context, Guid companyId, CancellationToken cancellationToken = default)
    { if (Company(context, companyId) is null) return []; await using var db = Create(context); var effects = await db.TaxAccountingEffects.AsNoTracking().Where(item => item.CompanyId == companyId).ToListAsync(cancellationToken); var journals = await db.Journals.AsNoTracking().Include(item => item.Lines).Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); var rules = await db.PostingRules.AsNoTracking().Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); return effects.Select(item => { var posted = 0m; var status = FinanceEvidenceStatus.PendingMapping; if (journals.TryGetValue(item.JournalId, out var journal) && rules.TryGetValue(item.PostingRuleId, out var rule)) { var taxAccount = item.Kind == FinanceOpenItemKind.Payable ? rule.DebitAccountId : rule.CreditAccountId; posted = journal.Lines.Where(line => line.AccountId == taxAccount).Sum(line => item.Kind == FinanceOpenItemKind.Payable ? line.FunctionalDebit : line.FunctionalCredit); status = item.ReversalJournalId is not null ? FinanceEvidenceStatus.Reversed : Math.Abs(posted - item.FunctionalAmount) <= 0.00000001m ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping; } return new FinanceTaxAccountingReconciliationRecord(item.Id, item.CompanyId, item.OpenItemId, item.TaxId, item.TaxAmount, posted, status, item.JournalId, item.ReversalJournalId); }).ToArray(); }
    public async Task<IReadOnlyList<FinanceFxReconciliationRecord>> ReconcileFxAsync(FinanceRequestContext context, Guid companyId, CancellationToken cancellationToken = default)
    { if (Company(context, companyId) is null) return []; await using var db = Create(context); var allocations = await db.Allocations.AsNoTracking().Where(item => item.CompanyId == companyId && item.RealizedFxAmount != 0m && item.ReversalOfAllocationId == null).ToListAsync(cancellationToken); var reversals = await db.Allocations.AsNoTracking().Where(item => item.CompanyId == companyId && item.ReversalOfAllocationId != null).ToListAsync(cancellationToken); var journals = await db.Journals.AsNoTracking().Include(item => item.Lines).Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); var rules = await db.PostingRules.AsNoTracking().Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); return allocations.Select(item => { var direction = item.RealizedFxDirection == "Gain" ? FinanceFxDirection.Gain : FinanceFxDirection.Loss; var reversal = reversals.SingleOrDefault(value => value.ReversalOfAllocationId == item.Id); var expectedAccount = item.RealizedFxRuleId is { } ruleId && rules.TryGetValue(ruleId, out var rule) ? direction == FinanceFxDirection.Gain ? rule.CreditAccountId : rule.DebitAccountId : (Guid?)null; var journal = item.RealizedFxJournalId is { } journalId && journals.TryGetValue(journalId, out var found) ? found : null; var posted = journal is null || expectedAccount is null ? 0m : journal.Lines.Where(line => line.AccountId == expectedAccount).Sum(line => direction == FinanceFxDirection.Gain ? line.FunctionalCredit : line.FunctionalDebit); var valid = journal is not null && expectedAccount is not null && Math.Abs(posted - item.RealizedFxAmount) <= 0.00000001m; var reversalValid = reversal?.JournalId is { } reversalJournalId && journal is not null && journals.TryGetValue(reversalJournalId, out var reverseJournal) && reverseJournal.ReversalOfJournalId == journal.Id && Math.Abs(reverseJournal.Lines.Where(line => line.AccountId == expectedAccount).Sum(line => direction == FinanceFxDirection.Gain ? line.FunctionalDebit : line.FunctionalCredit) - item.RealizedFxAmount) <= 0.00000001m; var status = reversal is not null ? (reversalValid ? FinanceEvidenceStatus.Reversed : FinanceEvidenceStatus.PendingMapping) : valid ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping; return new FinanceFxReconciliationRecord(item.Id, item.CompanyId, item.RealizedFxAmount, posted, direction, status, item.JournalId, item.OpenItemId, item.SettlementDocumentId, reversal?.JournalId, expectedAccount, item.RealizedFxRuleId, item.RealizedFxRuleVersionNumber, valid ? null : "realized_fx_line_mismatch"); }).ToArray(); }

    public async Task<IReadOnlyList<FinanceUnrealizedFxReconciliationRecord>> ReconcileUnrealizedFxAsync(FinanceRequestContext context, Guid companyId, CancellationToken cancellationToken = default)
    { if (Company(context, companyId) is null) return []; await using var db = Create(context); var lines = await db.RevaluationLines.AsNoTracking().Where(item => item.CompanyId == companyId).ToListAsync(cancellationToken); var journals = await db.Journals.AsNoTracking().Include(item => item.Lines).Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); var rules = await db.PostingRules.AsNoTracking().Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); return lines.Select(line => { var rule = line.PostingRuleId is { } ruleId && rules.TryGetValue(ruleId, out var foundRule) ? foundRule : null; var expected = rule is null ? (Guid?)null : line.Direction == FinanceFxDirection.Loss ? rule.DebitAccountId : rule.CreditAccountId; var journal = line.JournalId is { } journalId && journals.TryGetValue(journalId, out var foundJournal) ? foundJournal : null; var posted = journal is null || expected is null ? 0m : journal.Lines.Where(value => value.AccountId == expected).Sum(value => line.Direction == FinanceFxDirection.Loss ? value.FunctionalDebit : value.FunctionalCredit); var valid = journal is not null && expected is not null && Math.Abs(posted - Math.Abs(line.Difference)) <= 0.00000001m; var reversed = line.ReversalJournalId is { } reversalId && journal is not null && journals.TryGetValue(reversalId, out var reversal) && reversal.ReversalOfJournalId == journal.Id && Math.Abs(reversal.Lines.Where(value => value.AccountId == expected).Sum(value => line.Direction == FinanceFxDirection.Loss ? value.FunctionalCredit : value.FunctionalDebit) - Math.Abs(line.Difference)) <= 0.00000001m; var status = line.ReversalJournalId is not null ? (reversed ? FinanceEvidenceStatus.Reversed : FinanceEvidenceStatus.PendingMapping) : valid ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping; return new FinanceUnrealizedFxReconciliationRecord(line.Id, line.BatchId, line.CompanyId, line.SourceId, line.SourceType, Math.Abs(line.Difference), posted, line.Direction, status, line.JournalId, line.ReversalJournalId, expected, line.PostingRuleId, line.PostingRuleVersionNumber, valid ? null : "unrealized_fx_line_mismatch"); }).ToArray(); }

    public async Task<IReadOnlyList<FinanceReportingCurrencyReconciliationRecord>> ReconcileReportingCurrencyAsync(FinanceRequestContext context, Guid companyId, CancellationToken cancellationToken = default)
    {
        if (Company(context, companyId) is null) return [];
        await using var db = Create(context);
        var journals = await db.Journals.AsNoTracking()
            .Where(item => item.CompanyId == companyId && (item.Status == FinanceJournalStatus.Posted || item.Status == FinanceJournalStatus.Reversed))
