...
public sealed class FinanceMesp134Persistence : IFinanceMesp134Persistence
{
    ...
    { if (Company(context, companyId) is null) return []; await using var db = Create(context); var effects = await db.TaxAccountingEffects.AsNoTracking().Where(item => item.CompanyId == companyId).ToListAsync(cancellationToken); var journals = await db.Journals.AsNoTracking().Include(item => item.Lines).Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); var rules = await db.PostingRules.AsNoTracking().Where(item => item.CompanyId == companyId).ToDictionaryAsync(item => item.Id, cancellationToken); return effects.Where(item => journals.TryGetValue(item.JournalId, out var journal) && (asOfDate is null || journal.PostingDate <= asOfDate)).Select(item => { var journal = journals[item.JournalId]; var reversalVisible = asOfDate is null || (item.ReversalJournalId is { } reversalId && journals.TryGetValue(reversalId, out var reversalJournal) && reversalJournal.PostingDate <= asOfDate); var posted = 0m; var status = FinanceEvidenceStatus.PendingMapping; if (rules.TryGetValue(item.PostingRuleId, out var rule)) { var taxAccount = item.Kind == FinanceOpenItemKind.Payable ? rule.DebitAccountId : rule.CreditAccountId; posted = journal.Lines.Where(line => line.AccountId == taxAccount).Sum(line => item.Kind == FinanceOpenItemKind.Payable ? line.FunctionalDebit : line.FunctionalCredit); status = item.ReversalJournalId is not null && reversalVisible ? FinanceEvidenceStatus.Reversed : Math.Abs(posted - item.FunctionalAmount) <= 0.00000001m ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping; } return new FinanceTaxAccountingReconciliationRecord(item.Id, item.CompanyId, item.OpenItemId, item.TaxId, item.TaxAmount, posted, status, item.JournalId, item.ReversalJournalId is not null && reversalVisible ? item.ReversalJournalId : null); }).ToArray(); }
