...
public sealed class FinanceMesp134Persistence : IFinanceMesp134Persistence
{
    ...
    public async Task<IReadOnlyList<FinanceTaxAccountingReconciliationRecord>> ReconcileTaxAsync(FinanceRequestContext context, Guid companyId, DateOnly? asOfDate, CancellationToken cancellationToken = default)
    {
        ...
        return effects
            ...
            .Select(item =>
            {
                ...
                var journalEffective = journalFound && IsJournalEffective(journal!, asOfDate);
                if (!journalEffective)
                {
                ...
                if (rules.TryGetValue(item.PostingRuleId, out var rule))
                {
                    ...
                    FinanceJournalEntity? reversalJournal = null;
                    var reversalFound = item.ReversalJournalId is { } reversalId && journals.TryGetValue(reversalId, out reversalJournal);
                    var reversalEffective = reversalFound && IsJournalEffective(reversalJournal!, asOfDate);
                    var reversalValid = reversalEffective
                        && reversalJournal!.ReversalOfJournalId == journal!.Id
                        && Math.Abs(reversalJournal.Lines.Where(line => line.AccountId == taxAccount).Sum(line => item.Kind == FinanceOpenItemKind.Payable ? line.FunctionalCredit : line.FunctionalDebit) - item.FunctionalAmount) <= 0.00000001m;
                    var reversalExpectedAtAsOf = item.ReversalJournalId is not null
                        && (asOfDate is null || !reversalFound || reversalJournal!.PostingDate <= asOfDate);
                    status = reversalExpectedAtAsOf ? reversalValid ? FinanceEvidenceStatus.Reversed : FinanceEvidenceStatus.PendingMapping : Math.Abs(posted - item.FunctionalAmount) <= 0.00000001m ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping;
                }
                Guid? visibleReversalId = item.ReversalJournalId is { } candidateId && journals.TryGetValue(candidateId, out var candidateReversal) && IsJournalEffective(candidateReversal, asOfDate) ? candidateId : null;
                return new FinanceTaxAccountingReconciliationRecord(item.Id, item.CompanyId, item.OpenItemId, item.TaxId, item.TaxAmount, posted, status, item.JournalId, visibleReversalId);
            })
            .ToArray();
    }
    ...
    public async Task<IReadOnlyList<FinanceFxReconciliationRecord>> ReconcileFxAsync(FinanceRequestContext context, Guid companyId, DateOnly? asOfDate, CancellationToken cancellationToken = default)
    {
        ...
        return allocations
            ...
            .Select(item =>
            {
                ...
                var reversalJournal = reversal?.JournalId is { } reversalJournalId && journals.TryGetValue(reversalJournalId, out var foundReversalJournal) ? foundReversalJournal : null;
                var reversalVisible = asOfDate is null || (reversal is not null && reversal.AllocationDate <= asOfDate && (reversalJournal is null || IsJournalEffective(reversalJournal, asOfDate)));
                ...
                var journal = item.RealizedFxJournalId is { } journalId && journals.TryGetValue(journalId, out var found) && IsJournalEffective(found, asOfDate) ? found : null;
                ...
                var reversalValid = reversalVisible && reversalJournal is not null && journal is not null && IsJournalEffective(reversalJournal, asOfDate) && reversalJournal.ReversalOfJournalId == journal.Id && Math.Abs(reversalJournal.Lines.Where(line => line.AccountId == expectedAccount).Sum(line => direction == FinanceFxDirection.Gain ? line.FunctionalDebit : line.FunctionalCredit) - item.RealizedFxAmount) <= 0.00000001m;
    ...
    public async Task<IReadOnlyList<FinanceUnrealizedFxReconciliationRecord>> ReconcileUnrealizedFxAsync(FinanceRequestContext context, Guid companyId, DateOnly? asOfDate, CancellationToken cancellationToken = default)
    {
        ...
        return lines
            ...
            .Select(line =>
            {
                var rule = line.PostingRuleId is { } ruleId && rules.TryGetValue(ruleId, out var foundRule) ? foundRule : null;
                var expected = rule is null ? (Guid?)null : line.Direction == FinanceFxDirection.Loss ? rule.DebitAccountId : rule.CreditAccountId;
                var journal = line.JournalId is { } journalId && journals.TryGetValue(journalId, out var foundJournal) && IsJournalEffective(foundJournal, asOfDate) ? foundJournal : null;
                var reversal = line.ReversalJournalId is { } reversalId && journals.TryGetValue(reversalId, out var foundReversal) ? foundReversal : null;
                var reversalEffective = reversal is not null && IsJournalEffective(reversal, asOfDate);
                var reversalExpectedAtAsOf = IsReversalExpectedAtAsOf(line.ReversalJournalId, reversal, asOfDate);
                var posted = journal is null || expected is null ? 0m : journal.Lines.Where(value => value.AccountId == expected).Sum(value => line.Direction == FinanceFxDirection.Loss ? value.FunctionalDebit : value.FunctionalCredit);
                var valid = journal is not null && expected is not null && Math.Abs(posted - Math.Abs(line.Difference)) <= 0.00000001m;
                var reversed = reversalEffective && journal is not null && reversal!.ReversalOfJournalId == journal.Id && Math.Abs(reversal.Lines.Where(value => value.AccountId == expected).Sum(value => line.Direction == FinanceFxDirection.Loss ? value.FunctionalCredit : value.FunctionalDebit) - Math.Abs(line.Difference)) <= 0.00000001m;
                var status = reversalExpectedAtAsOf ? (reversed ? FinanceEvidenceStatus.Reversed : FinanceEvidenceStatus.PendingMapping) : valid ? FinanceEvidenceStatus.Reconciled : FinanceEvidenceStatus.PendingMapping;
                return new FinanceUnrealizedFxReconciliationRecord(line.Id, line.BatchId, line.CompanyId, line.SourceId, line.SourceType, Math.Abs(line.Difference), posted, line.Direction, status, line.JournalId, reversalEffective ? line.ReversalJournalId : null, expected, line.PostingRuleId, line.PostingRuleVersionNumber, valid ? null : "unrealized_fx_line_mismatch");
            }).ToArray(); }
    ...
    private static bool IsJournalEffective(FinanceJournalEntity journal, DateOnly? asOfDate) =>
        journal.Status is FinanceJournalStatus.Posted or FinanceJournalStatus.Reversed
        && (asOfDate is null || journal.PostingDate <= asOfDate.Value);

    private static bool IsReversalExpectedAtAsOf(Guid? reversalId, FinanceJournalEntity? reversalJournal, DateOnly? asOfDate) =>
        reversalId is not null && (asOfDate is null || reversalJournal is null || reversalJournal.PostingDate <= asOfDate.Value);
