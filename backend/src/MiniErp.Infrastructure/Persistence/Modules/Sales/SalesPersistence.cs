...
public sealed class SalesPersistence(DbContextOptions options) : ISalesPersistence
{
    ...
    public async Task<SalesOperationResult<SalesDeliveryResponse>> CompleteDeliveryAsync(ProcurementRequestContext context, Guid deliveryId, IReadOnlyList<Guid> movementIds, string idempotencyKey, string requestFingerprint, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await db.Deliveries.SingleOrDefaultAsync(item => item.Id == deliveryId, cancellationToken); if (entity is null || !InScope(context, entity.CompanyId, entity.BranchId)) return Failure<SalesDeliveryResponse>("delivery_not_found"); var recordedMovements = JsonSerializer.Deserialize<IReadOnlyList<Guid>>(entity.MovementIdsJson, Json) ?? []; if (entity.Status == SalesDeliveryStatus.Posted) return recordedMovements.SequenceEqual(movementIds.Distinct()) ? SalesOperationResult<SalesDeliveryResponse>.Success(ToDelivery(entity)) : Failure<SalesDeliveryResponse>("delivery_effect_mismatch"); if (entity.Status is not (SalesDeliveryStatus.Draft or SalesDeliveryStatus.Unknown)) return Failure<SalesDeliveryResponse>("delivery_not_postable"); if (movementIds is null || movementIds.Count == 0 || movementIds.Distinct().Count() != movementIds.Count) return Failure<SalesDeliveryResponse>("delivery_effect_invalid");
    ...
    public async Task<SalesOperationResult<SalesInvoiceRequestResponse>> CreateInvoiceRequestAsync(ProcurementRequestContext context, SalesInvoiceRequestWriteModel model, string idempotencyKey, string requestFingerprint, CancellationToken cancellationToken = default)
    {
        ...
        var priorAllocations = existingInvoices.SelectMany(item => ReadInvoiceLines(item.LinesJson).Evidence.SelectMany(evidence => evidence.Allocations)).ToArray();
        ...
        if (requestedByLine.Count != model.Lines.Count || requestedByLine.Keys.Any(id => order.LinesJson is null)) return Failure<SalesInvoiceRequestResponse>("invalid_invoice_lines");
        var evidenceByLine = (model.LineEvidence ?? []).ToDictionary(item => item.OrderLineId);
        var allocatedEvidence = new List<SalesInvoiceLineEvidence>();
        foreach (var requested in requestedByLine)
        {
            var orderLine = Lines(order.LinesJson).SingleOrDefault(item => item.Id == requested.Key);
